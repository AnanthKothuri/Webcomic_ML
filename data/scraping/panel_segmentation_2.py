...
def segment_panel_2(im):
    ...
    for segment_line in segment_lines[0]:              
        # iterate through first line to look at all cases
        p0, p1 = segment_line
        for segment_line2 in segment_lines[0]:
            if(in_line(segment_line, segment_line2, 1)):
                new_line = merge_line(segment_line, segment_line2, 1, distance)
                if (new_line != segment_line): #
                    if segment_line in segment_lines[0]:
                        segment_lines[0].remove(segment_line)
                    if segment_line2 in segment_lines[0]:
                        segment_lines[0].remove(segment_line2)
                    segment_lines[0].append(new_line)
    ...
    for cutting_line in cutting_lines[0]:
   ...
   # Extrapolating and merging vertical lines
        ...
        try:
            if verticalcuts(horizontal_c_pos[i], horizontal_c_pos[i+1], vertical_c_lines) != False:
                ...
                for j in range(len(cutting_points)-1):
                    ...
                    panels += [[horizontal_c_pos[i], horizontal_c_pos[i+1],cutting_points[j], cutting_points[j+1]]]
                ...
                #print("saved", filenames[image_i][:-4] + "_" + str(order_ctr) )
                panels += [[horizontal_c_pos[i], horizontal_c_pos[i+1],1, im_width-1]]
    ...
    # for i in reversed(range(1, len(panels) - 1)):
    #     prev = panels[i+1]
    #     next = panels[i+1]
    #     cur = panels[i]

    #     if (np.abs(prev[0] - cur[0]) < error and np.abs(prev[1] - cur[1]) and np.abs(prev[2] - cur[2]) and np.abs(prev[3] - cur[3])):
    #         panels.remove(prev)
    #     if (np.abs(next[0] - cur[0]) < error and np.abs(next[1] - cur[1]) and np.abs(next[2] - cur[2]) and np.abs(next[3] - cur[3])):
    #         panels.remove(next)
